...
function Card({ animationDelay }: { animationDelay: number }) {
  return (
    <li
      style={{ animationDelay: `${777 + animationDelay * 200}ms` }}
      className={`relative inline-block mr-2 md:mr-6 h-[40rem] w-96 md:w-100 2xl:w-xl lg:w-lg  md:h-[80%] rounded-xl shadow-lg  overflow-hidden drop-shadow-md ${styles.card}  `}
    >
      {/* background layer */}
      {/* <div
        style={{
      ...
      <div className=" w-[36rem] h-[50rem] rounded-xl bg-white/6 p-12 isolate aspect-video opacity-100 z-10">
        <a className="absolute right-8 top-8 rounded-sm p-2 bg-white flex items-center justify-center cursor-pointer ring-2 ring-gray-200/80 drop-shadow-md">
        ...
        <div className="absolute inset-y-60 md:inset-y-70 m-auto">
          ...
          <div className="flex flex-wrap w-90">
            <div className="flex items-center justify-between bg-gray-100 rounded-xl md:rounded-2xl px-4 h-6 md:h-8 w-fit mr-2 mb-2">
